<script lang="ts">
	...
	async function deleteService(serviceId: string) {
		try {
			...
			acts.add({ mode: 'success', message: '✓ deleted service successfuly!', lifetime: 2 });
			location.reload();
		} catch {
			acts.add({ mode: 'danger', message: 'failed to delete service!', lifetime: 2 });
		}
	}
</script>
				...
				<tr>
					...
					<td class="td-action">
						<div class="row-action-wrapper">
							...
							<button data-variant="ghost" onclick={() => deleteService(service.id)}>
								<BxTrash size="18" color="var(--primary)" />
							</button>
